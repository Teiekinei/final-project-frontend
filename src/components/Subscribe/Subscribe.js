...
import { getMovieGenres, postSubscribedGenresToDB } from "../../WebAPI";
...
const Subscribe = styled(MySubscribeButton)`
  margin-left: 88%;
  width: auto;
...
const HomeNewsletter = styled.div`
  ...
  font-family: "微軟正黑體";
  box-shadow: rgba(0, 0, 0, 0.3) 5px 5px 15px, rgba(0, 0, 0, 0.3) -5px -5px 15px,
    rgba(0, 0, 0, 0.3) 5px -5px 15px, rgba(0, 0, 0, 0.3) -5px 5px 15px;
  & .single {
    max-width: 650px;
    margin: 0 auto;
    text-align: center;
  ...
  function handleOnChecked(e) {
    ...
    if (e.target.checked) {
      for (let i = 0; i < genres.length; i++) {
        copyGenres.push(genres[i]);
      }
      copyGenres.push(e.target.name);
      setGenres(copyGenres);
    } else {
      copyGenres = genres.filter((genre) => genre !== e.target.name);
      setGenres(copyGenres);
    }
  }

  async function handleOnSubmit() {
    if (email === "" || genres.length === 0) {
      alert("電子郵件或訂閱類型不得為空!");
      return;
    }
    ...
    if ((await postSubscribedGenresToDB(email, genres)) === 1) {
      alert("訂閱成功!");
    }
  }
